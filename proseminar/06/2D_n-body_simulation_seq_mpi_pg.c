...
int main(int argc, char **argv) {
    ...
    int particle_count = 1;  // particles
    int max_Mass = 10000;
    ...
    *min_y = 0, *max_y = 0;
    ...
    if (print && rank == 0) {
        // printf( "Starting n-body simulation for Nx = %lld, Ny = %lld and "
        // "particle_count = " "%d\n", Nx, Ny, particle_count);
        printf(
            "Sending from rank 0 to rank 1 with one particle of mpi "
            "struct\n");
    }
    if (rank == 0) {
        // Send MPI
        printf("The send particle has the following information:\n");
        printf("Pos_x: %f, Pos_y: %f\n", P[0].pos.x, P[0].pos.y);
        printf("Vel_x: %f, Vel_y: %f\n", P[0].vel.x, P[0].vel.y);
        printf("Mass:%f\n", P[0].mass);

        MPI_Send(&P[0], 1, mpi_particle, 1, 42, MPI_COMM_WORLD);

    } else {
        // Receive MPI and show values
        particle recv_part;
        MPI_Recv(&recv_part, 1, mpi_particle, 0, 42, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
        printf("The received particle has the following information:\n");
        printf("Pos_x: %f, Pos_y: %f\n", recv_part.pos.x, recv_part.pos.y);
        printf("Vel_x: %f, Vel_y: %f\n", recv_part.vel.x, recv_part.vel.y);
        printf("Mass:%f\n", recv_part.mass);
    }
