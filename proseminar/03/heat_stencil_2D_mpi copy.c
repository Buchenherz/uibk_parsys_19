...
int main(int argc, char **argv)
{
  ...
  for (int t = 0; t < T; t++)
  {
    ...
    Vector received_upper_row = createVector(N);
    Vector received_lower_row = createVector(N);

    if (!(rank % 2)) {
      if (lower_rank >= 0) {  // 1 (from 2)
        /* A lower rank exists, receive one from below and send one to below */
        MPI_Ssend(A[local_last_row_index], 1, row, lower_rank, 0, comm_cart);
        MPI_Recv(received_lower_row, 1, row, lower_rank, 0, comm_cart, MPI_STATUS_IGNORE);
        A[local_last_row_index + 1] = received_lower_row;
        if (A[local_last_row_index][0] < 270.0) {
          printf("T: %d rank: %d send_lower_row temp = %lf\n",t,rank, A[local_last_row_index][0]);
        }
        if (A[local_last_row_index + 1][0] < 270.0) {
          printf("T: %d rank: %d rec_lower_row temp = %lf\n",t,rank, A[local_last_row_index + 1][0]);
        }
      }
      if (upper_rank >= 0) { // 3 (from 2)
        /* A upper rank exists, receive one from above and send one to above */
        MPI_Ssend(A[local_first_row_index], 1, row, upper_rank, 0, comm_cart);
        MPI_Recv(received_upper_row, 1, row, upper_rank, 0, comm_cart, MPI_STATUS_IGNORE);
        A[local_first_row_index - 1] = received_upper_row;
        if (A[local_first_row_index][0] < 270.0) {
          printf("T: %d rank: %d send_upper_row temp = %lf\n",t,rank,A[local_first_row_index][0]);
        }
        if (A[local_first_row_index + 1][0] < 270.0) {
          printf("T: %d rank: %d rec_upper_row temp = %lf\n",t,rank,A[local_first_row_index+1][0]);
        }
      }
      /* Received the last local row from the lower rank, appending it as the new last local row */
    }
    else {
      if (upper_rank >= 0) {  // 2 (from 1)
        /* A upper rank exists, receive one from above and send one to above */
        MPI_Recv(received_upper_row, 1, row, upper_rank, 0, comm_cart, MPI_STATUS_IGNORE);
        MPI_Ssend(A[local_first_row_index], 1, row, upper_rank, 0, comm_cart);
        A[local_first_row_index - 1] = received_upper_row;
      ...
      releaseVector(received_lower_row);
      releaseVector(received_upper_row);
...
int *getRowSizePerRank(int M, int number_of_ranks)
{
  ...
  {
    rows_per_rank[loop] = whole_div;
    if (loop == 0) 
      rows_per_rank[loop] += M % number_of_ranks;
  }
