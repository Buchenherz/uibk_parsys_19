...
int main(int argc, char **argv) {
    ...
    if (argc > 3) {
        ...
        M = atoi(argv[2]);
        L = atoi(argv[3]);
        if (argc > 4) print = strcmp("print", argv[4]) == 0;
    } else {
        printf(
            "usage: matrix_mul_seq <int Matrix1 rows> <int Matrix1 columns && "
            "Matrix2 rows> <int Matrix2 columns> <string print>\n");
    }
...
#pragma omp parallel for schedule(static, 10) shared(C) firstprivate(A, B)
    // The static, 10 is the reason our results show the similarities in the
    // results between 4 and 8. We are always deviding by 100 which leads to
    // load imbalance in the later parts because of division. If the outer loop
    // is smaller than the inner loop, collapse is faster?
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < L; j++) {
            for (int k = 0; k < M; k++) {
                C[i][j] += A[i][k] * B[k][j];
