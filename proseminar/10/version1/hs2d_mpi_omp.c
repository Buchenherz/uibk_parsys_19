...
int main(int argc, char **argv) {
    ...
    MPI_Init_thread(&argc, &argv, MPI_THREAD_FUNNELED, &provided);  // initialize the MPI environment
    ...
    for (int t = 0; t < T; t++) {
        ...
        if (upper_rank < 0) {
            ...
            //              received_lower_row, 1, row, lower_rank, 0,
            ...
            // MPI_STATUS_IGNORE); MPI_Sendrecv(A[local_last_row_index], 1, row,
            // lower_rank, 0, received_lower_row, 1, row, lower_rank, 0,
            // comm_cart, MPI_STATUS_IGNORE);
            MPI_Isend(A[local_first_row_index], 1, row, upper_rank, 0,
                      comm_cart, &request);
            MPI_Isend(A[local_last_row_index], 1, row, lower_rank, 0, comm_cart,
