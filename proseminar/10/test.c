...
int main(int argc, char **argv)
{
  ...
  int subSize[2] = {M, N};
  int start[2] = {0, 0};
  MPI_Type_create_subarray(2, size, subSize, start, MPI_ORDER_C, MPI_DOUBLE, &subarrayType);
  ...
  {
    local_m_start = local_m_start + rank_row_sizes[i];
  }
  local_m_end = local_m_start + rank_row_sizes[rank];
    ...
    if (!(t % 1000) && print)
    {
      if (rank == 0)
      {
        ...
        for (int orank = 1; orank < number_of_ranks; orank++)
        {
          ...
          value_t recv[M][N];
          // printf("\nRecv array %d\n", orank);
          MPI_Recv(&recv, 1, subarrayType, orank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
